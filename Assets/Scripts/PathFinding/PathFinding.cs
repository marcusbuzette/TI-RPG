    ...
    public void Setup(int width, int height, float cellSize) {
        ...
        gridSystem = new GridSystem<PathNode>(width, height, cellSize,
            (GridSystem<PathNode> g, GridPosition gridPosition) => new PathNode(gridPosition),
             LevelGrid.Instance.GetZoneList());
    ...
    private List<PathNode> GetNeighbourList(PathNode currentNode) {
        ...
        if (gridPosition.x - 1 >= 0) {
            ...
            if (LevelGrid.Instance.GetGameMode() == LevelGrid.GameMode.EXPLORE ||
                GetNode(gridPosition.x - 1, gridPosition.z + 0).GetGridPosition().zone == gridPosition.zone) {
                neighbourList.Add(GetNode(gridPosition.x - 1, gridPosition.z + 0));
            }
            if (gridPosition.z - 1 >= 0) {
                // Left Down node
                if (LevelGrid.Instance.GetGameMode() == LevelGrid.GameMode.EXPLORE ||
                    GetNode(gridPosition.x - 1, gridPosition.z - 1).GetGridPosition().zone == gridPosition.zone) {
        ...
        if (gridPosition.z - 1 >= 0) {
            // Down node
            if (LevelGrid.Instance.GetGameMode() == LevelGrid.GameMode.EXPLORE ||
                GetNode(gridPosition.x - 0, gridPosition.z - 1).GetGridPosition().zone == gridPosition.zone) {
        ...
        if (gridPosition.z + 1 < gridSystem.GetHeight()) {
            // Up node
            if (LevelGrid.Instance.GetGameMode() == LevelGrid.GameMode.EXPLORE || GetNode(gridPosition.x - 0, gridPosition.z + 1).GetGridPosition().zone == gridPosition.zone) {
                neighbourList.Add(GetNode(gridPosition.x - 0, gridPosition.z + 1));
            }
        }
